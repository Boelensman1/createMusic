 ...
 */
   ...
   */
        function run(key, selfie) {
          ...
          for (let i = 0, type, what; i < options[key].length; i++) {
            what = options[key][i];
            type = typeof what;
  ...
  //
        if (typeof args[args.length - 1] !== 'function') {
          return function emitter() {
            ...
            return self.emit(...args.concat(arg));
          };
        }
...
//
      ...
      let EventEmitter = _dereq_('eventemitter3'),
 ...
 */
      Recovery.prototype.reconnect = function reconnect() {
        ...
        return recovery.backoff((err, opts) => {
 ...
 */
  ...
  //
    ...
    : opts.min;
        ...
        recovery.timers.setTimeout('reconnect', () => {
          opts.duration = (+new Date()) - opts.start;
          opts.backoff = false;
    ...
    // and usable API.
    //
          const connect = recovery._fn = one((err) => {
            recovery.reset();
          ...
          recovery.timers.setTimeout('timeout', () => {
...
//
      ...
      let required = _dereq_('requires-port'),
 ...
 */
      function Primus(url, options) {
        ...
        if (typeof this.client !== 'function') {
          const message = 'The client library has not been compiled correctly, ' +
 ...
 */
  ...
  .on('reconnect failed', primus.emits('reconnect failed', (next) => {
    primus.emit('end');
    next();
  }))
  .on('reconnect timeout', primus.emits('reconnect timeout'))
  .on('reconnect scheduled', primus.emits('reconnect scheduled'))
  .on('reconnect', primus.emits('reconnect', (next) => {
    primus.emit('outgoing::reconnect');
    next();
  }));

        primus.on('outgoing::open', () => {
          const readyState = primus.readyState;
    ...
    //
        ...
        primus.on('incoming::error', (e) => {
          let connect = primus.timers.active('connect'),
             err = e;
    ...
    //
        ...
        primus.on('incoming::data', (raw) => {
          primus.decoder(raw, (err, data) => {
      //
 ...
 */
  ...
  //
        ...
        //
          ...
          transformer.call(connection, packet, (err, arg) => {
 ...
 */
   ...
   */
        ...
        primus.timers.setTimeout('connect', () => {
 ...
 */
      Primus.prototype.merge = function merge(target) {
        for (let i = 1, key, obj; i < arguments.length; i++) {
          obj = arguments[i];
...
//
  ...
  //
    ...
    //
          try {
             let prot = primus.url.protocol === 'ws+unix:' ? 'ws+unix:' : 'ws:',
          qsa = prot === 'ws:';
    ...
    //
          ...
          socket.onmessage = primus.emits('incoming::data', (next, evt) => {
  ...
  //
        primus.on('outgoing::data', (message) => {
